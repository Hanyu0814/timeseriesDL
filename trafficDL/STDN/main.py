...
parser.add_argument('--dataset', type=str, default='i10', help='i10')
...
class AccLossHistory(keras.callbacks.Callback):
    # custom callback for recording loss and acc per batch
...
def main(batch_size=64, max_epochs=100, validation_split=0.2, early_stop=EarlyStopping()):
    ...
    if args.dataset == 'i10':
        ...
        raise Exception("Can not recognize dataset, please enter i10")
    modeler = models.models()

    if args.model_name == "stdn":
        # training
        att_cnnx, att_x, cnnx, x, y = sampler.sample_stdn(datatype="train",
        ...
        # his_list = [history.history['loss'], history.history['val_loss'], history.history['acc'], history.history['val_acc']]
        # np.save('history_acc_loss.npy', np.array(his_list))
            
        att_cnnx, att_x, cnnx, x, y = sampler.sample_stdn(datatype="test", nbhd_size=args.nbhd_size,
                                                                          cnn_nbhd_size=args.cnn_nbhd_size)
        
        y_pred = model.predict( \
            x=att_cnnx +  att_x + cnnx +  [x, ], )
        threshold = float(sampler.threshold)
        denorm_y = y * sampler.config["volume_max"]
        denorm_y_pred = y_pred * sampler.config["volume_max"]
        print("Evaluating threshold: {0}.".format(threshold))
        (prmse, pmape), (drmse, dmape) = eval_lstm(denorm_y, denorm_y_pred, threshold)
        print(
            "Test on model {0}:\neast rmse = {1}, east mape = {2}%\nwest rmse = {3}, west mape = {4}%".format(
...
if __name__ == "__main__":
    stop = CustomStopper(monitor='val_loss', min_delta=0, patience=5, verbose=0, mode='min', start_epoch=5)
    #main(batch_size=args.batch_size, max_epochs=args.max_epochs, early_stop=stop)
    main(batch_size=args.batch_size, max_epochs=20, early_stop=stop)
